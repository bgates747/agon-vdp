...
enum {
    PINGO_CLIP_OUTSIDE_NEAR = 1 << 0,
    PINGO_CLIP_OUTSIDE_FAR = 1 << 1,
    PINGO_CLIP_OUTSIDE_LEFT = 1 << 2,
    PINGO_CLIP_OUTSIDE_RIGHT = 1 << 3,
    PINGO_CLIP_OUTSIDE_BOTTOM = 1 << 4,
    PINGO_CLIP_OUTSIDE_TOP = 1 << 5,
    PINGO_CLIP_PLANES = (1 << 6) - 1,
    PINGO_CLIP_MAX_VERTICES = 9,
};

typedef struct PingoClipVertex {
    Vec4f position;
    Vec2f texture;
} PingoClipVertex;
...
static inline int pingoClipVertexFinite(PingoClipVertex vertex) {
    return isfinite(vertex.position.x) && isfinite(vertex.position.y) &&
        isfinite(vertex.position.z) && isfinite(vertex.position.w) &&
        isfinite(vertex.texture.x) && isfinite(vertex.texture.y);
}

static inline float pingoClipDistance(PingoClipVertex vertex, uint8_t plane) {
    switch (plane) {
...
static inline PingoClipVertex pingoClipInterpolate(
        PingoClipVertex from, PingoClipVertex to, float amount,
        uint8_t plane) {
    PingoClipVertex result = {
        .position = {
            ...
            from.position.w + (to.position.w - from.position.w) * amount,
        },
        .texture = {
            from.texture.x + (to.texture.x - from.texture.x) * amount,
            from.texture.y + (to.texture.y - from.texture.y) * amount,
        },
    };
    switch (plane) {
        case PINGO_CLIP_OUTSIDE_NEAR:
...
static inline int pingoClipSamePosition(PingoClipVertex a,
                                       PingoClipVertex b) {
...
static inline int pingoClipAppendUnique(PingoClipVertex *output,
...
static inline uint8_t pingoClipTriangle(
    ...
    for (uint8_t plane_index = 0;
         plane_index < sizeof(planes) / sizeof(planes[0]); ++plane_index) {
        ...
        float previous_distance = pingoClipDistance(previous, plane);
        int previous_inside = previous_distance >= 0.0f;
        for (uint8_t index = 0; index < count; ++index) {
            PingoClipVertex current = output[index];
            float current_distance = pingoClipDistance(current, plane);
            ...
            if (previous_inside != current_inside) {
                ...
                PingoClipVertex intersection = pingoClipInterpolate(
                    previous, current, amount, plane);
                if (!pingoClipVertexFinite(intersection) ||
                    !pingoClipAppendUnique(scratch, &next_count,
                                           intersection)) {
                    return 0;
                }
            }
            if (current_inside &&
                !pingoClipAppendUnique(scratch, &next_count, current)) {
                return 0;
            }
            previous = current;
        ...
        if (next_count > 1 &&
            pingoClipSamePosition(scratch[0], scratch[next_count - 1])) {
            --next_count;
        }
        if (next_count < 3) {
            return 0;
