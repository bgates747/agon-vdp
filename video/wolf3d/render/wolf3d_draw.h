...
private:
	...
	// buffers fail safe as solid wall rather than reading out of bounds. Pingo
	// controls deliberately occupy ordinary buffer storage, so reject their
	// bytes here rather than interpreting a live C++ object as a Wolf tilemap.
	// This remains a use-time check so later ordinary recreation of this ID is
	// immediately visible to the existing Wolf scene.
	uint8_t GetTile(int tilex, int tiley) const {
		if (isPingo3dControlBuffer(m_world.tilemapBufferId)) {
			return WOLF3D_TILE_WALL_MIN;
		}
