...
void IRAM_ATTR VDUStreamProcessor::vdu_sys_buffered() {
	...
	switch (command) {
		...
		case BUFFERED_COMPRESS_SZIP: {
			auto sourceBufferId = readWord_t();
			if (sourceBufferId == -1) return;
			bufferCompressSzip(bufferId, sourceBufferId);
		}	break;
...
// Compress (blocks from) a buffer into a new buffer using 'szip' compression.
// Replaces the target buffer with the new one.
//
void VDUStreamProcessor::bufferCompressSzip(uint16_t bufferId, uint16_t sourceBufferId) {
}

// VDU 23, 0, &A0, bufferId; &43, sourceBufferId; : Decompress blocks from a buffer
