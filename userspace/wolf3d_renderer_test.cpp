...
std::unordered_set<std::uint16_t> pingo3dControlBuffers;

bool isPingo3dControlBuffer(std::uint16_t bufferId) {
...
void testPingoControlCannotMasqueradeAsTilemap() {
	Fixture f;
	f.setTile(4, 4, 7);
	const auto x = 2 * WOLF3D_TILEGLOBAL + 0x8000;
	const auto y = 4 * WOLF3D_TILEGLOBAL + 0x8000;

	pingo3dControlBuffers.insert(kTilemapBufferId);
	f.cast(x, y, 0);
	expect("Pingo tilemap isolation", "safe wall tile",
	       WOLF3D_TILE_WALL_MIN, f.renderer.WallTiles()[kCenterColumn]);

	pingo3dControlBuffers.erase(kTilemapBufferId);
	f.cast(x, y, 0);
	expect("ordinary tilemap recovery", "wall texture", 7,
	       f.renderer.WallTiles()[kCenterColumn]);
}
...
int main() {
	...
	std::puts("wolf3d_renderer_test: door, typed tilemap, actor rotation/lifetime, depth sort, and sprite occlusion invariants pass");
	return 0;
}
